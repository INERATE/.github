<div align="center">

<img src="https://raw.githubusercontent.com/INERATE/.github/main/profile/logo-dark.png" alt="INERATE" width="340" />

### Iterate until done.

**We build the systems that make AI agents engineer like principals —**
**done-state contracts, design law, and structure instead of vibes.**

<br/>

[![Stars](https://img.shields.io/github/stars/INERATE/atelier?style=flat-square&labelColor=0A0A0B&color=10B981&label=stars)](https://github.com/INERATE/atelier/stargazers)
[![CI](https://img.shields.io/github/actions/workflow/status/INERATE/atelier/ci.yml?style=flat-square&labelColor=0A0A0B&color=10B981&label=ci)](https://github.com/INERATE/atelier/actions)
[![Last commit](https://img.shields.io/github/last-commit/INERATE/atelier?style=flat-square&labelColor=0A0A0B&color=9B9B98&label=last%20commit)](https://github.com/INERATE/atelier/commits/main)
[![Issues](https://img.shields.io/github/issues/INERATE/atelier?style=flat-square&labelColor=0A0A0B&color=9B9B98&label=issues)](https://github.com/INERATE/atelier/issues)
[![PRs](https://img.shields.io/github/issues-pr/INERATE/atelier?style=flat-square&labelColor=0A0A0B&color=9B9B98&label=PRs)](https://github.com/INERATE/atelier/pulls)
[![License](https://img.shields.io/github/license/INERATE/atelier?style=flat-square&labelColor=0A0A0B&color=9B9B98&label=license)](https://github.com/INERATE/atelier/blob/main/LICENSE)

</div>

---

## What we believe

LLMs don't fail because they can't code. They fail because nobody told them
what **done** means, what **good** looks like, and when to **stop**. Inerate
exists to close that gap: every loop needs a base case —

```
if (done) return;
```

## What we build

| Project | What it is |
|---------|------------|
| **[Atelier](https://github.com/INERATE/atelier)** | The craftsman's framework for AI-built software: loop engineering with base cases, enforced clean-code law, anti-AI-slop design law, automatic model routing, SQLite knowledge graph, self-learning overlays. One install. |

```
/plugin marketplace add INERATE/atelier
/plugin install atelier
/atelier:bootstrap
```

## How we engineer

- **Done-states, not vibes** — every task terminates against a written,
  checkable contract.
- **Laws, not suggestions** — 250 words per file enforced by hooks; one accent
  color enforced by review; no polling, anywhere, ever.
- **Token economy** — knowledge lives in queryable rows and graphs, not
  re-read documents.
- **Premium by default** — if it looks like a template, we delete it.

## Built in the open

<div align="center">

[![Contributors](https://img.shields.io/github/contributors/INERATE/atelier?style=flat-square&labelColor=0A0A0B&color=10B981&label=contributors)](https://github.com/INERATE/atelier/graphs/contributors)
[![Commit activity](https://img.shields.io/github/commit-activity/m/INERATE/atelier?style=flat-square&labelColor=0A0A0B&color=9B9B98&label=commits%2Fmonth)](https://github.com/INERATE/atelier/graphs/commit-activity)
[![Repo size](https://img.shields.io/github/repo-size/INERATE/atelier?style=flat-square&labelColor=0A0A0B&color=9B9B98&label=size)](https://github.com/INERATE/atelier)

[Contribution leaders](https://github.com/INERATE/atelier/graphs/contributors) ·
[Star history](https://github.com/INERATE/atelier/stargazers) ·
[Activity pulse](https://github.com/INERATE/atelier/pulse) ·
[Good first issues](https://github.com/INERATE/atelier/contribute)

</div>

## Contributing

Every repo ships with a contribution guide, and every contribution passes the
same gates our frameworks enforce on generated code. Start with an issue —
we answer.

---

<div align="center">

Founded and led by <a href="https://github.com/ScienHAC"><b>Piyush Sharma</b></a> (owner) · MIT licensed · Built in the open

</div>
